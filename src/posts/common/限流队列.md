...
按照MDN文档的说法，先去日志查看429响应的详情头有没有`Retry-After`请求头，如果有的话那么限流条件就根据这个请求的值来设定，没有的话就自己定一个限制条件
...
Go通过`container/list`包来模拟队列，同时通过`sync`包和`time`包来实现并发控制，代码如下：
...
Go在主协程退出之后不论子协程是否在运行都会直接被销毁，因此在`main`函数的代码最后加了一个睡眠，让子协程执行结束之后再退出（但真实HTTP服务器主协程正常情况下不可能退出哈）。
...
解决思路还是挺简单的，代码实现比较难，需要对TS和Go的异步编程要有一定的理解，TS和Go的异步实现感觉还是差距挺大，写的时候两边思路不能完全套用。



> 封面来源：[Pixiv](https://www.pixiv.net/artworks/71187447)
>
> 文档参考：[MDN Web Docs]([MDN Web Docs (mozilla.org)](https://developer.mozilla.org/zh-CN/))